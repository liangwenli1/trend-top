...
function LanguageTooltip({active,payload,l,t,total}){if(!active||!payload?.length)return null;const item=payload[0].payload;return <div className="analytics-tooltip analytics-language-tooltip"><strong>{item.name==='Other'?t.other:item.name}</strong><span>{num(item.count,l)} {t.projects}{total>0&&` · ${new Intl.NumberFormat(l==='zh'?'zh-CN':'en-US',{style:'percent',maximumFractionDigits:1}).format(item.count/total)}`}</span></div>}
