...
function LanguageTooltip({active,payload,l,t}){if(!active||!payload?.length)return null;const item=payload[0].payload;return <div className="analytics-tooltip analytics-language-tooltip"><strong>{item.name==='Other'?t.other:item.name}</strong><span>{num(item.count,l)}</span></div>}
