...
export function ResourceGuide({ item, l }) {
  ...
  useEffect(() => { setCopied(false); setError(false); }, [item.id, item.install]);
  const fields=[];
  if(item.type==='skill')fields.push([zh?'资源文件':'Resource file',signals.resourcePath]);
  if(item.type==='plugin')fields.push([zh?'传输方式':'Transport',signals.transport],[zh?'运行要求':'Runtime requirements',signals.runtime]);
  if(item.type==='agent')fields.push([zh?'运行要求':'Runtime requirements',signals.runtime]);
  if(item.type==='components')fields.push([zh?'框架':'Framework',signals.framework]);
  ...
  return <section className="resource-guide"><h2>{labels[item.type]}</h2><dl>{fields.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value?(safeSourceUrl(value)?<a href={safeSourceUrl(value)} target="_blank" rel="noopener noreferrer">{value}</a>:<span>{value}</span>):<span className="muted">{zh?'来源尚未提供，查看原始资料确认。':'Not provided by the source. Check the original documentation.'}</span>}</dd></div>)}</dl>
    {item.install && <div className="resource-installation"><div className="resource-installation-heading"><span>{zh ? '安装 / 使用说明' : 'Installation / usage'}</span><small>{zh ? '来自原始资源' : 'From the source'}</small></div><div className="resource-command-box"><pre className="resource-instruction"><code>{item.install}</code></pre><button type="button" className="resource-copy" aria-label={copied ? (zh ? '已复制' : 'Copied') : (zh ? '复制使用说明' : 'Copy instructions')} title={copied ? (zh ? '已复制' : 'Copied') : (zh ? '复制' : 'Copy')} onClick={async () => { try { await navigator.clipboard.writeText(item.install); setCopied(true); setError(false); } catch { setError(true); } }}>{copied ? <Check size={20}/> : <Copy size={20}/>}</button></div>{copied && <span className="resource-copy-status" role="status">{zh ? '已复制' : 'Copied'}</span>}{error && <p role="alert">{zh ? '无法访问剪贴板，请手动复制。' : 'Clipboard unavailable. Copy the instructions manually.'}</p>}</div>}
