...
A runnable bilingual discovery site for open-source skills, plugins, agents, components, websites, and GitHub repositories. Browsing, search, rankings, charts, compare, and detail pages are free. The daily email digest is a **Pro** feature. The default is **DEMO mode**: metadata and 33 days of metrics are illustrative synthetic samples. The interface and API label them as demo data. They are not GitHub's current rankings.

The site stores metrics in **PostgreSQL** when `DATABASE_URL` is set, and falls back to embedded **PGlite** for local demo and tests. Rankings are computed in SQL from pre-aggregated `period_metrics`. The first live collect expands 12 weeks of official Star history into daily rows, so day/week/month Star gains work immediately. Windows end on the last complete UTC day before the collection run; the sample day itself is never counted as a zero gain. Fork net still needs a second daily snapshot.
...
Switching to production requires production products, API key, webhook secret, `mode=prod`, `https://api.creem.io`, an HTTPS `PUBLIC_URL`, and a real end-to-end payment test. Prices in Admin are display values in major units with up to two decimals (`9.99`), set separately for `/en` and `/zh`; Creem charges the product's own price, so keep the two in sync.
...
Run `npm run collect` at about 02:00 UTC daily with a system scheduler. Run `npm run digest` hourly. After a schema change or a restore, `npm run backfill` rebuilds `daily_metrics` and `period_metrics`. The collector uses deterministic query families rather than random sampling: popular, recently created, language, topic, and content-type queries each have a reserved candidate budget. It records returned, deduplicated, accepted, rate-limit, and error counts in `sync_query_stats`, which an administrator can inspect from the account menu. The collector also stores the discovery query and last-seen time on every record; a record missed by the same successful query for three runs becomes inactive and returns automatically if rediscovered.

Independent Website sources live in `website_sources`. The starter registry covers 21st.dev, Skills.sh, Smithery, Glama, Agent Skills, and shadcn/ui. Website metadata is collected directly or through Firecrawl when configured in Site administration, with robots checks, a small request interval, and exponential retry delay. When a Firecrawl key is set, collect also extracts skills.sh install counts and Glama usage/downloads onto matching GitHub Skill/plugin assets. `npm run history` refreshes official Star history without running discovery. The sample is a bounded candidate universe (up to 2,000 repositories and 400 assets per type), not all of GitHub. `GET /api/admin` and `POST /api/admin/collect`, `/digest`, or `/backfill` require an administrator account session (the same cookie as the account page) for status and manual reruns; use `./deploy.sh collect` from the server shell instead of curl.

Set `PERF_LOG=1` to emit structured API performance records. Site administration also exposes rolling route P50/P95 latency, database time, query count, response size, and error rate.
