...
登录后右上角显示黑色方框中的邮箱首字母按钮，展开账户菜单。管理员与普通用户使用同一套登录；主导航不会多出 Admin。未登录访问账户页面会转到 `/login`，登录后返回原来的站内目标页面。
...
## 2026-09-16：邮件停止流程与 Pricing 加载修正

- 邮件正文的“管理邮件推送”链接指向 `/{locale}/account/delivery`，“停止邮件推送”指向同一页面的 `?intent=stop`。
- 未登录先进入独立 `/login`，登录后返回邮件设置页。已经登录则直接管理当前账户的邮件。
- 旧 `/manage?token=…` 和 `/unsubscribe?token=…` 页面兼容跳转，不在页面打开时改变推送状态，也不使用邮件令牌自动登录。
- 点击“停止邮件推送”打开确认弹窗；明确说明不会改变 Pro 和付费续订。确认才执行认证后的状态更新；错误可重试，关闭弹窗恢复按钮焦点。
- 邮箱客户端自带的一键退订继续使用 `List-Unsubscribe` 的 POST 端点，与正文的登录管理链接分开。
- Pricing 采用按语言隔离的两分钟内存缓存，共享进行中的请求；网站预取公开套餐，管理员保存设置时清空缓存。缓存过期或浏览器刷新仍会请求最新套餐。请求失败提供重试，不一直停在 Loading plans。
- Subscribe 与 Pricing 导航暂时保留，待用户选择方案。推荐主导航只保留 Pricing；首页保留邮件摘要介绍和入口，已登录用户从账户菜单进入邮件推送设置。备选是将 Subscribe 改名 Daily digest，专门介绍邮件功能，Pricing 专门负责套餐购买。

验证：28 项自动测试及生产构建通过；本地浏览器验证旧链接登录回跳、打开页面保持发送中、取消确认、确认停止、Pricing 重复访问和首字母头像。仅使用本地测试账户，未操作生产账单。
