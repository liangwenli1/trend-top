...
- Billing: admin settings require an HttpOnly admin session; secrets are encrypted and redacted; public plans expose one Pro tier with monthly/yearly cycles; unauthenticated checkout/portal requests fail; invalid plan keys fail; a signed `checkout.completed` event activates Pro once; duplicate events are harmless; Account opens Creem Customer Portal for its own customer only.
